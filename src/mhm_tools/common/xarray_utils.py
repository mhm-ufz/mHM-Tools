...
def get_coord_key(ds, lat=False, lon=False, raise_exception=True, is_retry=False):
    ...
    if not is_retry and isinstance(ds, xr.Dataset):
        logger.warning(f"{type(ds)} does not contain fitting coordinates. Trying again looking for dimensions")
        return get_coord_key(ds, lat=lat, lon=lon, raise_exception=raise_exception, is_retry=True)
    if raise_exception:
        with ErrorLogger(logger):
...
def get_single_data_var(ds):
    ...
    if len(data_vars) > 1: 
        logger.error(f"Only single data_var allowed")
        return None
