...
class Hydrograph:
    ...
    def check_which_plots_to_create(self, a):
        ...
        return (0,0,0,0,0)
    def gen_hydrograph(self, input_path, filename, show, save, title, plots):
        """
        ...
        plots = self.check_which_plots_to_create(plots)
        if sum(plots) == 0:
            return
        with xr.open_dataset(input_path + "discharge.nc") as ds:
            ...
            if plots[0]:
                self.logger.info('generating daily discharge plot')
                r, c = self.get_row_col()
                ax1 = fig.add_subplot(gs[r, :])
                ...
                ax1.set_xlim(discharge_daily["time"][0],discharge_daily["time"][-1])

            if plots[1]:
                self.logger.info('generating monthly discharge plot')
                r, c = self.get_row_col()
                if r == 0:
                    ax2 = fig.add_subplot(gs[r, :])
                    self.grid[r] = [True for c in self.grid[r]]
                else:
            ...
            if plots[3]:
                ...
                self.plot_on_axis(
                    ...
                    xvalues=range(1, 13),
                    yvalues=[
                        self.get_long_time_monthly_mean(discharge_daily["sim"]),
                        self.get_long_time_monthly_mean(discharge_daily["obs"]),
                    ],
                    s=1,
                )
                self.plot_on_axis(
            ...
            if plots[4]:
                self.logger.info('generating daily discharge scatter plot')
                r, c = self.get_row_col()
                if r == 0:
                    ax5 = fig.add_subplot(gs[r, :])
                    self.grid[r] = [True for c in self.grid[r]]
                else:
                ...
                # -----------WINKELGERADE---------------------
                ax5.plot([0, 290], [0, 290], c="lightgrey", linewidth=0.5, zorder=0)
                ...
                ax5.scatter(discharge_daily['obs'], discharge_daily['sim'], s=50, c='black', edgecolor='white', linewidth=0.3, alpha=0.1)
                # s = size of dots , c = 'green', marker='x' = shape of dots
                ax5.set_xlabel("Qobs [m\N{SUPERSCRIPT THREE}/s]")  # X Achsenbeschriftung
                ax5.set_ylabel("Qsim [m\N{SUPERSCRIPT THREE}/s]")  # Y Achsenbeschriftung
                ax5.set_xlim(0, np.max(discharge_daily['obs']) * 1.1)
                ax5.set_ylim(0, np.max(discharge_daily['sim']) * 1.1)
                # ----------REGRESSIONSLINIE------------------
                # obtain m (slope) and b(intercept) of linear regression line
                m, b = np.polyfit(discharge_daily['obs'], discharge_daily['sim'], deg=1)
                # add linear regression line to scatterplot
                plt.plot(discharge_daily['obs'], m * discharge_daily['obs'] + b, color='red', linewidth=0.7)
            plt.tight_layout()
            if save:
