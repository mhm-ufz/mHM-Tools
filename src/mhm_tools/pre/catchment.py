...
def get_upscaling_factor(resolutions, max_resolution=False, l1=False, l2=True):
    ...
    input_res = resolutions.l0_resolution
    if l1:
        upscale_res = resolutions.l1_resolution
    elif l2:
        upscale_res = resolutions.l2_resolution
    else:
    ...
    not_int_multiple_msg = f"Upscaling only works if L1 resolution is integer muplipe of L0 resolution but L1 = {self.resolutions.l1_resolution / input_res:.4f} * L0"
...
def align_bounds_to_l2(ds, resolutions, min_row, max_row, min_col, max_col):
    ...
    lon = ds.lon.values
    lat = ds.lat.values
    tol = resolutions.l0_resolution / 2 + 1e-9
    ...
    def _idx_for(coordinate_values, target_values, name):
        ...
        target = target_values + resolutions.l0_resolution / 2 * asc_factor
...
def cut_to_filled_area(
    ...
    if factor > 1:
        logger.info(
            f"Regridding to fit coarse grid with res {max([r for r in [resolutions.l1_resolution, resolutions.l11_resolution, resolutions.l2_resolution] if r is not None ])} (factor {factor})"
        )

        if resolutions.l2_file is not None and not repeat:
            logger.debug(
                f"Aligning to L2 grid from file {resolutions.l2_file}"
            )
    ...
    if factor > 1 and ((n_lat % factor) != 0 or (n_lon % factor) != 0):
        ...
        if (
            ...
            return cut_to_filled_area(ds=ds, resolutions=resolutions, catchment_mask=catchment_mask, buffer=buffer, repeat=True)
        with ErrorLogger(logger):
...
class Catchment:
    ...
    def __init__(
        ...
        self.resolutions.l0_resolution = round(
            abs(ds.lon.data[1] - ds.lon.data[0]), l0_presision
        )
    ...
    def _distance_100m_units(self, di, dj, lat_deg=None):
        """Backwards-compatible wrapper around the shared distance helper."""
        return distance_100m_units(
            di=di,
            dj=dj,
            l0_resolution=self.resolutions.l0_resolution,
            lat_deg=lat_deg,
            latlon=self.latlon,
        )
    ...
    def find_best_gauge_location(
        ...
        """Backwards-compatible wrapper to find the best outlet cell."""
    ...
    def get_best_gauge_coordinate(
        ...
        if ref_catchment_area is not None:
            if method != "all":
                outlet_idx, error, distance_error = find_best_gauge_location(
                    self.ds,
                    upstream_area,
                    gauge_coords,
                    ref_catchment_area,
                ...
                try:
                    ...
                    logger.warning(
                        f"Error occurred while finding BasinEx candidate: {e}"
                    )
                    outlet_idx_bx, error_bx, distance_error_bx = (None, None, None)
                try:
                    ...
                    logger.warning(f"Error occurred while finding Burek candidate: {e}")
                    if outlet_idx_bx is not None:
                        logger.info(
                            "Using BasinEx candidate since Burek candidate failed."
                        )
                        return outlet_idx_bx, error_bx, distance_error_bx
                    logger.warning(
                        "Both BasinEx and Burek candidates failed; falling back to original gauge coordinates."
                    )
    ...
    def upscale(self, var):
        ...
        logger.info(
            f"Upscaling flow direction to {self.resolutions.l1_resolution} with the fator {factor}."
        )
    ...
    def processing_data_variable(
        ...
        if (
            ...
            lon = np.arange(
                lon.min() - input_res / 2 + self.resolutions.l1_resolution / 2,
                lon.max() + self.resolutions.l1_resolution / 2,
                self.resolutions.l1_resolution,
            )
            lat = np.arange(
                lat.max() + input_res / 2 - self.resolutions.l1_resolution / 2,
                lat.min() - self.resolutions.l1_resolution / 2,
                -self.resolutions.l1_resolution,
            )
        logger.debug(
            f"lon_min {np.min(lon):.3f}, lon_max {np.max(lon):.3f}, resulution: {self.resolutions.l1_resolution}"
        )
    ...
    def write_mask_file(self, ds, mask_file):
        ...
        if mask_file is not None:
            ...
            elif self.resolutions.l2_resolution is not None:
